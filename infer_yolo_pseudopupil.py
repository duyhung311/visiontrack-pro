...
# If a custom data_dir is provided, put the output CSV inside a subdirectory of csv_outputs matching the folder name
if args.data_dir:
    clean_data_dir = args.data_dir.rstrip("\\/")
    folder_name = os.path.basename(clean_data_dir)
    if folder_name and folder_name not in ["visible", "mat_files", "images", "labels", "", ".", ".."]:
        csv_dir, csv_file = os.path.split(OUTPUT_CSV)
        if "csv_outputs" in csv_dir or csv_dir == "csv_outputs" or csv_dir == "./csv_outputs" or csv_dir.endswith("csv_outputs"):
            OUTPUT_CSV = os.path.join(csv_dir, folder_name, csv_file)
        else:
            OUTPUT_CSV = os.path.join("./csv_outputs", folder_name, csv_file)
